...
export async function registerRoutes(
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      const isValidPassword = process.env.DATABASE_URL 
        ...
        : true; // In demo mode, accept any password for any user
  ...
  // Staff: Update certificate
  app.patch("/api/certificates/:id", requireAuth, upload.single("file"), async (req, res) => {
    try {
      const parsed = insertCertificateSchema.partial().safeParse({
        ...req.body,
      ...
      const certificate = await storage.updateCertificate(req.params.id, parsed.data);
      if (!certificate) {
        return res.status(404).json({ error: "Certificate not found" });
      }
      res.json(certificate);
